---
title: '**Lab 3: Optical satellite imagery**'
author: '*Hans Ole Ørka, 4. September 2026*'
output:
  html_notebook: 
    toc: true
    toc_depth: 3
...
**Data:** The data for the lab can be be downloaded from the link below. The data include a GeoPackage with field reference data of dominant species (Spruce, Pine, Broadleaves) and AGB (Mg/ha) and an area of interest polygon (AOI).

- https://eduumb.sharepoint.com/:f:/s/team_mina305instructors/IgDQvt_W2pV1R5HfIFdT2CGeAdrRPM7_O-uKhNYUjWTDVUs?e=mWPNFP 

**Note:** Download the folders separately. Before decompressing (Extract all) the folder "S2B_MSIL2A_20220624T104629_R051_T32VNM_20220624T122953.SAFE", rename it "S2" to avoid error caused by long path name on Windows.
...
![](03_OpticalSatelliteImagery.drawio.png "")


## 1. Download Sentinel-2 data 
- Download Sentinel-2 data from https://browser.dataspace.copernicus.eu/ or use the data provided in the sharepoint. The field inventory was carried out in summer 2022.
...
library(sf)
gpkg <- "data/lab3.gpkg" # Path of geopackage 
...
The Sentinel-2 bands are stored separately. Create multiband images at 10 and 20 m resolution covering the aoi and write to disk. Functions for listing files (*dir*), *rast* and *crop* functions needed. If there are problems with downloading data (approximately 1 GB), the results from this step can be downloaded from: https://eduumb.sharepoint.com/:f:/s/team_mina305instructors/IgDQvt_W2pV1R5HfIFdT2CGeAdrRPM7_O-uKhNYUjWTDVUs?e=mWPNFP  
...
f1 <- dir(path,pattern="_B",full.names=T) # list all files containing _B
f1
# Read all files
s <- rast(f1)               
s2_10m <- crop(s,aoi) # Clip raster to aoi
names(s2_10m) <- c("Blue","Green","Red","NIR") # Rename bands
names(s2_10m) <- paste0(names(s2_10m),"_10m") # Concatenate resolution's name to band, to make it unique
...
plot(ndvi)
e2 <- extract(ndvi,prfl) # Extract NDVI at sample plot locations 
...
We need to standardize resolutions to make predictions for individual pixels. This is solved by resampling the 20 m images to 10 m and then stack them into one raster. 

```{r}
# Resample and merge 10 m and 20 m Sentinel 
s2_20m_resampled <- resample(s2_20m,s2_10m) # Resample
s2 <- c(s2_10m,s2_20m_resampled) # Stack
s2
```
...
dat$Species <- factor(dat$Species,levels=c("Spruce","Pine","Broadleaves")) # or if you do not care about the order of species, just: as.factor(data$Species)
```
...
We will use the measurements of biomass (AGB) on the sample plots to make a model to predict biomass. We will evaluate the prediction accuracy using the Root Mean Square Error. RMSE is calculated as the square root of the mean of the squared differences between observed values ($y_i$) and predicted values ($\hat{y_i}$) for a dataset of size *n*:
$$
RMSE = \sqrt{\frac{1}{n} \sum_{i=1}^{n}(y_i - \hat{y}_i)^2}
$$
Then we will make a scatter plot of the observed (y-axis) vs predicted values (fitted) (x-axis). For reasoning behind this see Piñeiro et al. (2008) [^1]. 
